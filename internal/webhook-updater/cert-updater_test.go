...
var _ = Describe("Test file processing", func() {
	...
	It("should not patch if no file exists", func() {
		...
		defer close(stop)
		go updater.processFileChanges(clientProxy, stop)
	...
	It("should exit if webhook is removed", func() {
		writeCertificate(tempDir, certPemData1)
		...
		go func() {
			updater.processFileChanges(clientProxy, stop)
			close(exited)
		}()
...
func (c *clientProxy_mock) CheckWebhookExists(name string) bool {
